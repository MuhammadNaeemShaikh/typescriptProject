...
class Customer {
  constructor(
    ...
    public mobNumber: number, // Use lowercase 'number' instead of 'Number'
...
// Bank class
class Bank {
  customers: Customer[] = []; // Use a better variable name, e.g., 'customers'
  accounts: BankAccount[] = []; // Use a better variable name, e.g., 'accounts'
  ...
  do {
    ...
    switch (service.select) {
      case 'View Balance': {
        ...
        //find account
        const isAccountNumber = HBL.accounts.find(
          (val) => val.accNumber == accNum.num
        );
      ...
      case 'Cash Withdrawl': {
        ...
        //find account
        let isAccountNumber = HBL.accounts.find(
          (val) => val.accNumber == accNum.num
        );
        if (!isAccountNumber) {
          console.log('Wrong Credientials');
          ...
          if (
            cashWithDraw.num > 500 &&
            isAccountNumber.balance > cashWithDraw.num
          ) {
            isAccountNumber.balance -= cashWithDraw.num;

            console.log(
              `Your Remaining Account Balance is ${isAccountNumber.balance}`
            );
          } else {
            console.log(
              'WithDrawl amount should be more than 500 and account should have enough balance'
            );
          }
        }
